...
.W .sr,.D .sr{flex-direction:row;align-items:center;gap:10px}
.W .sr label,.D .sr label{flex:0 0 46%;font-size:11px}
.W .sr select,.D .sr select{flex:1}
...
.W .frow,.D .frow{flex-direction:row;align-items:center;gap:10px}
.W .frow label,.D .frow label{flex:0 0 46%;font-size:12px}
.W .frow input,.D .frow input{flex:1}
...
function lk(gas,inP,diam){
  ...
  c.sort(function(a,b){return(a.id_mm-b.id_mm)||(a.max_pressure-b.max_pressure);});return c[0];
}
function ft(s){return s.replace(/X/g," \u00d7 ").replace(/\s{2,}/g," ").trim();}
function sc(gas,inP,diam){
  var m=lk(gas,inP,diam);
  if(!m)return"<div class='rbox er'>\u26a0 No approved tube specification found</div>";
  var badge="";
  if(gas==="O2")badge="<span class='badge bo2'>O\u2082 \u2192 S14 only</span>";
  if(gas==="H2")badge="<span class='badge bh2'>H\u2082 \u2192 S6/S9</span>";
  return"<div class='spc'><div class='sh'>\u25ba Recommended Tube Specification"+badge+"</div>"
    +"<div class='sr2'>\u2713&nbsp;<strong>Tube Spec:&nbsp;&nbsp;&nbsp;&nbsp;</strong>"+m.spec+"<br>"
    +"\u2713&nbsp;<strong>Min. Tube Size:&nbsp;</strong>"+ft(m.tube_od_w)+"</div>"
    +"<div class='sn'>ID "+m.id_mm.toFixed(3)+" mm&nbsp;\u2502&nbsp;Max rated: "+m.max_pressure+" bar</div></div>";
}
...
function calc(){
  ...
  try{
    ...
    else{rn.innerHTML="";rc.innerHTML="<div style='font-size:13px;color:var(--tc2);line-height:1.85'>"+line+"<br>Recommended Spec: <strong>"+(spec?spec.spec:"N/A")+"</strong><br>"+(spec?"Tube: <strong>"+ft(spec.tube_od_w)+"</strong> (ID "+spec.id_mm.toFixed(2)+" mm)":"")+"</div>";}
    showC(pct);
