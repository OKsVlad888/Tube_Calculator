...
Layout (matches screenshot exactly):
  • HUD background: full 100vw x 100vh, all animations unchanged
  • Empty black rectangle in HUD: left≈1%, top≈21%, width≈54%, height≈44%
  • Calculator panel: centred inside that rectangle, with internal scroll
  • All text: 16px (Word size 16)
  • Page: fits browser window exactly — NO outer scroll ever
...
# Hide all Streamlit chrome; make the components iframe cover the full viewport
st.markdown("""
<style>
...
/* ================================================================
   RESET + PAGE
   The entire page is 100vw x 100vh.  NOTHING outside the
   calculator panel ever scrolls.
   ================================================================ */
...
html, body {
  ...
  font-size: 16px;           /* Word size-16 base */
  color: #00e5cc;
}

/* ================================================================
   HUD IFRAME  – full-screen background, z-index 1
   ================================================================ */
#hud {
  position: fixed;
  inset: 0;
  ...
  height: 100%;
  border: none;
  pointer-events: none;   /* HUD never blocks mouse events */
  z-index: 1;
}

/* ================================================================
   WRAPPER
   Matches the empty black rectangle drawn by the HUD:
     left ≈ 1%   width ≈ 54%   (right edge ≈ 55%)
     top  ≈ 21%  height ≈ 44%  (bottom edge ≈ 65%)
   Using flexbox to centre the panel perfectly inside it.
   ================================================================ */
#wrapper {
  position: fixed;
  left:   1%;
  ...
  z-index: 500;
  pointer-events: none;    /* transparent outside the panel */
}

/* ================================================================
   CALCULATOR PANEL
   • Fills 96% of the wrapper in both dimensions
   • overflow-y:auto  → internal scroll only
   • overflow-x:hidden → no horizontal scroll
   • pointer-events:all → captures all mouse/keyboard events
   ================================================================ */
#panel {
  pointer-events: all;
  width:      96%;
  ...
  background: rgba(0, 8, 8, 0.93);
  border: 1.5px solid #00e5cc;
  border-radius: 4px;
  padding: 14px 22px 18px;

  box-shadow:
    0 0 22px rgba(0, 229, 204, 0.42),
...
@keyframes panelGlow {
  ...
  to   { box-shadow: 0 0 28px rgba(0,229,204,0.66), inset 0 0 52px rgba(0,229,204,0.07); }
}
...
/* ================================================================
   PANEL CONTENTS  – all z-index:1 to sit above scanlines
   ================================================================ */
...
<!-- HUD background (full-screen, z-index 1, pointer-events:none) -->
%%HUD%%

<!-- Centering wrapper over the HUD's empty black rectangle -->
<div id="wrapper">

  <!-- Calculator panel with internal scroll -->
  <div id="panel">
    ...
    <div id="body">
      ...
      <div id="input-fields"><!-- built by JS --></div>
...
// ── Field lists per calculation type ────────────────────────────────────
const FIELDS = {
...
function fv(label) {
  const el = document.getElementById(toKey(label));
...
function calcDiameter(Pi,Po,Tc,L,Q,gas) {
  ...
  return Math.pow(
    (FR*L*8*rhoAvg(Pi,Po,Tc,gas)*(Q/60000)**2) / (Math.PI**2*dP),
    0.2
  ) * 1000;
}
function calcFlow(Pi,Po,Tc,L,D,gas) {
  ...
  if (dP <= 0) throw new Error("Inlet pressure must exceed outlet pressure.");
  return Math.sqrt(
    dP * Math.PI**2 * (D/1000)**5 / (8*FR*L*rhoAvg(Pi,Po,Tc,gas))
  ) * 60000;
}
function calcLength(Pi,Po,Tc,D,Q,gas) {
  const dP = (Pi-Po)*1e5;
  if (dP <= 0) throw new Error("Inlet pressure must exceed outlet pressure.");
  return dP * Math.PI**2 * (D/1000)**5 /
         (8*FR*rhoAvg(Pi,Po,Tc,gas)*(Q/60000)**2);
}
function calcOutlet(Pi,Tc,L,D,Q,gas) {
  const Dm = D/1000, Qs = Q/60000;
  function res(Po) {
    return (Pi-Po)*1e5 -
           (8*FR*L*rhoAvg(Pi,Po,Tc,gas)*Qs**2) / (Math.PI**2*Dm**5);
  }
  let lo=0, hi=Pi;
  for (let i=0; i<60; i++) {
...
function calcInlet(Po,Tc,L,D,Q,gas) {
  ...
  for (let i=0; i<60; i++) {
    const m = (lo+hi)/2;
    const vm = calcOutlet(m,Tc,L,D,Q,gas);
...
function pipeSpec(D,P,gas) {
  ...
  if (D <= 4) {
    ...
    rows=[['Special piping','Outside standard range']]; rec='Special';
  }
  return { rec, rows };
}
...
function renderResult(mainLine, D, P, gas) {
  ...
  return "<div class='rbox ok'>"
       + mainLine
       + "<br><small>Recommended: " + rec + "</small>"
       + "</div>"
       + "<p class='sp-title'>Possible pipe specifications:</p>"
       + "<table class='stbl'>"
       + "<thead><tr><th>Pipe Spec</th><th>Range</th></tr></thead>"
       + "<tbody>" + tr + "</tbody>"
       + "</table>";
}

// ── Calculate (called on button click) ───────────────────────────────────
function doCalc() {
  const gas  = document.getElementById("gasSelect").value;
  const ct   = document.getElementById("calcSelect").value;
  const area = document.getElementById("result-area");
  try {
    const Tc = fv("Temperature (\u00b0C)");
    const Pi = fv("Inlet Pressure (bar)");
    const Po = fv("Outlet Pressure (bar)");
    const L  = fv("Pipe Length (m)");
    const D  = fv("Pipe Diameter (mm)");
    const Q  = fv("Flow Rate (LPM)");
    let html = "";
...
// ── Rebuild input fields when calc type changes ───────────────────────────
